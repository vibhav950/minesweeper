  ...
  for (int i = 0; i < board->rows; i++)
  {
    ...
    for (int j = 0; j < board->cols; j++)
    {
      if (state != MSW_NONE)
      {
        ...
        {
          printf("%s%s%d ", BG_CYAN_LIGHT, FG_MAGENTA, cells[i * board->cols + j].value);
        }
      }
      ...
      else
      {
        ...
        else
        {
          ...
          printf("%s%s%d ", bg, FG_MAGENTA, cells[i * board->cols + j].value);
        }
      }
    }
    printf("%s\n", FG_BG_CLEAR);
...
int main (void) {
  ...
  do
  {
    ...
    GenerateBoard(ctx);
    // Display(ctx, ctx->board->sx, ctx->board->sy);
