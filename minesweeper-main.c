...
void printd(int diff)
{
  ...
  {
  case 1:
    printf("\r%s%s EASY %s  MEDIUM   HARD ", BG_CYAN_LIGHT, FG_BLACK, FG_BG_CLEAR);
    break;
  case 2:
    printf("\r EASY  %s%s MEDIUM %s  HARD ", BG_CYAN_LIGHT, FG_BLACK, FG_BG_CLEAR);
    break;
  case 3:
    printf("\r EASY   MEDIUM  %s%s HARD %s", BG_RED_LIGHT, FG_BLACK, FG_BG_CLEAR);
    break;
  }
}
...
{
  MSW_BOARD *board = ctx->board;
  if (!board) printf("No board");

  SetBombs(ctx);

  ctx->cur_board = (MSW_CELL*) malloc(sizeof(MSW_CELL) * board->rows * board->cols);
...
//         else
//         {
//           printf("%s%s%d ",BG_CYAN_LIGHT, FG_MAGENTA_LIGHT, cells[i * board->cols + j].value);
//         }
//       }
//       else
//       {
...
//     printf("%s\n", FG_BG_CLEAR);
//   }
//   printf("\n    %s%sFLAGS%s %2d\t", BG_YELLOW, FG_BLACK, FG_BG_CLEAR, board->flags);
//   printf("%s%sTIME%s %4ds\n", BG_GREY, FG_BLACK, FG_BG_CLEAR, 0);
// }
  ...
  for (int i = 0; i < board->rows; i++)
  {
    ...
    for (int j = 0; j < board->cols; j++)
    {
      ...
      else
      {
        ...
        {
          bg = (i == sx && j == sy) ? BG_BLUE_LIGHT : BG_GRAY;
          printf("%s%s%d ", bg, FG_MAGENTA_LIGHT, cells[i * board->cols + j].value);
        }
      }
    }
    printf("%s\n", FG_BG_CLEAR);
...
void MultiClick (MSW_CTX *ctx, int x, int y)
{
  ...
  for (int i = -1; i <= 1; i++)
  {
    for (int j = -1; j <= 1; j++)
    {
      if (IsValidPos(board, x + i, y + j) && cells[(x + i) * board->cols + (y + j)].flag == 0)
      {
        Click(ctx, x + i, y + j);
      }
    }
...
int main (void) {
  ...
  do
  {
    if (!NewBoard(ctx)) {printf("exiting"); exit(2);}
